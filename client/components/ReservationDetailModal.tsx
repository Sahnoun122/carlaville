...
const Step = ({ active, label }: { active: boolean, label: string }) => (
  <div className="relative z-10 flex flex-col items-center gap-2">
     <div className={`w-8 h-8 rounded-full border-[3px] flex items-center justify-center transition-all duration-700 ${
        active ? 'bg-red-600 border-white scale-110 shadow-lg shadow-red-100' : 'bg-white border-gray-100 shadow-sm'
     }`}>
        {active && <CheckCircle2 className="w-4 h-4 text-white" />}
     </div>
     <span className={`text-[8px] font-black uppercase tracking-widest ${
        active ? 'text-neutral-900' : 'text-gray-300'
     }`}>{label}</span>
  </div>
);
